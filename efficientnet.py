...
def efficientnet_b0(pretrained=True):
    ...
    model = Efficientnet(width_coefficient=1.0,
                        ...
                        dropout_rate=0.2,
                        num_classes=1000)
    model_name = 'efficientnet-b0'  # 可以选择 'efficientnet-b0' 到 'efficientnet-b7'
...
def efficientnet_b1(pretrained=True):
    ...
    model = EfficientNet.from_pretrained(model_name) if pretrained else EfficientNet.from_name(model_name)
    if pretrained:
        pretrained_path = "/home/zhangyulong/NewFolder/efficientnet-b1-f1951068(1).pth"  
        state_dict = torch.load(pretrained_path)
        model.load_state_dict(state_dict)
...
def efficientnet_b2(pretrained=True):
    ...
    model = EfficientNet.from_pretrained(model_name) if pretrained else EfficientNet.from_name(model_name)
    if pretrained:
        pretrained_path = "/home/zhangyulong/NewFolder/efficientnet-b2-8bb594d6(1).pth"  
        state_dict = torch.load(pretrained_path)
        model.load_state_dict(state_dict)
    return model
...
def efficientnet_b3(pretrained=True):
    ...
    model = EfficientNet.from_pretrained(model_name) if pretrained else EfficientNet.from_name(model_name)
    if pretrained:
        pretrained_path = "/home/zhangyulong/NewFolder/efficientnet-b3-5fb5a3c3(1).pth"  
        state_dict = torch.load(pretrained_path)
        model.load_state_dict(state_dict)
    return model
...
def efficientnet_b4(pretrained=True):
    ...
    model = EfficientNet.from_pretrained(model_name) if pretrained else EfficientNet.from_name(model_name)
    if pretrained:
        pretrained_path = "/home/zhangyulong/NewFolder/efficientnet-b4-6ed6700e(1).pth"  
        state_dict = torch.load(pretrained_path)
        model.load_state_dict(state_dict)
...
def efficientnet_b5(pretrained=True):
    ...
    model = EfficientNet.from_pretrained(model_name) if pretrained else EfficientNet.from_name(model_name)
    if pretrained:
        pretrained_path = "/home/zhangyulong/NewFolder/efficientnet-b5-b6417697(1).pth"  
        state_dict = torch.load(pretrained_path)
        model.load_state_dict(state_dict)
...
def efficientnet_b6(pretrained=True):
    ...
    model = EfficientNet.from_pretrained(model_name) if pretrained else EfficientNet.from_name(model_name)
    if pretrained:
        pretrained_path = "/home/zhangyulong/NewFolder/efficientnet-b6-c76e70fd(1).pth"  
        state_dict = torch.load(pretrained_path)
        model.load_state_dict(state_dict)
    return model
...
def efficientnet_b7(pretrained=True):
    ...
    model = EfficientNet.from_pretrained(model_name) if pretrained else EfficientNet.from_name(model_name)
    if pretrained:
        pretrained_path = "/home/zhangyulong/NewFolder/efficientnet-b7-dcc49843(1).pth"  
        state_dict = torch.load(pretrained_path)
        model.load_state_dict(state_dict)
    return model
